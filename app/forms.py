...
class RegistrationForm(FlaskForm):
    ...
    region = SelectField('Регион', validators=[DataRequired()], choices=[],
                         render_kw={'class': 'searchable-select', 'id': 'region'})
    settlement = SelectField('Населённый пункт', validators=[DataRequired()], choices=[],
                             render_kw={'class': 'searchable-select', 'id': 'settlement'})
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.subjects.choices = [(s.id, s.name) for s in
                                 db.session.execute(sa.select(Subject)).scalars().all()]
        self.region.choices = [(r.id, r.name) for r in
...
class PageAddForm(FlaskForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.type_of_work.choices = [(tow.id, tow.name) for tow in
